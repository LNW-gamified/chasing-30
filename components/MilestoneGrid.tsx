...
export default function MilestoneGrid({
  ...
  const [stadiumCollectibles, setStadiumCollectibles] = useState<Array<{ id: string; name: string; category: string; giveaway_type: string | null; giveaway_quantity: string | null; photo_url: string | null; signed_by: string | null; acquired_from: string | null; rating: number | null; price: number | null; stadium_visit_id: string | null; baseball_life_entry_id: string | null }>>([])
  ...
  const fetchCollectibles = useCallback(async () => {
    const supabase = createClient()
    const { data } = await supabase.from('collectible_log').select('id, name, category, giveaway_type, giveaway_quantity, photo_url, signed_by, acquired_from, rating, price, stadium_visit_id, baseball_life_entry_id').order('created_at', { ascending: false })
