...
function computeRating(avgHigh: number, avgPrecipDays: number): 'great' | 'good' | 'fair' | 'avoid' {
  ...
  let level: number
  if (avgHigh >= 65 && avgHigh <= 80)                                          level = 3
  else if ((avgHigh >= 55 && avgHigh < 65) || (avgHigh > 80 && avgHigh <= 90)) level = 2
  else if ((avgHigh >= 45 && avgHigh < 55) || (avgHigh > 90 && avgHigh <= 95)) level = 1
  else                                                                           level = 0
  if (avgPrecipDays > 8 && level > 0) level--
  return LEVELS[level]
}

interface YearMonthBucket { temps: number[]; precipDays: number; winds: number[] }
...
export async function GET(req: NextRequest) {
  ...
  if (cached && cached.length === 12) {
    ...
    return NextResponse.json({ data: normalized.filter(r => BASEBALL_MONTHS.includes(r.month)) })
  }
  ...
  if (!stadium?.lat || !stadium?.lng) {
    return NextResponse.json({ error: 'Stadium coordinates not available' }, { status: 404 })
  }
